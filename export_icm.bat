...
REM Use PowerShell to read env var directly, avoiding batch intermediate variable
for /f "tokens=1-3* delims=	" %%a in ('powershell -NoProfile -Command "$file = $env:ITEMTYPE_LIST_FILE; $content = Get-Content $file; foreach($line in $content) { $parts = $line -split '\\|'; Write-Output ($parts[0] + [char]9 + $parts[1] + [char]9 + $parts[2]) }"') do (
